...
export const findQuestionByUuid = (
  ...
  try {
    ...
    for (const quiz of parsedData.data) {
      const question = quiz.setData.find((q: QuizQuestion) => q.questionUuid === questionUuid);
      if (question) {
        console.log("findQuestionByUuid - success: ", question);
        return question;
      }
    }
    ...
    console.error(
      "🎯event_log:  🎇/findQuestionByUuid  ❌ Error occurred while parsing data from local storage:",
      error
    );
